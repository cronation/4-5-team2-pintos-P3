...
load_segment (struct file *file, off_t ofs, uint8_t *upage,
	...
	while (read_bytes > 0 || zero_bytes > 0) {
		...
		// printf("[[TRG]]\n lzload_arg TRUE-FALSE\n");

		if (!vm_alloc_page_with_initializer (VM_ANON, upage,
					writable, lazy_load_segment, lzl)){
			// printf("[[TRG]]\n WITH_INITIALIZER_TRUE??\n");
			// 여기까지 출력됨
			return false;
			}
...
setup_stack (struct intr_frame *if_) {
	...
	// 새 page 만들기
	// stack bottom에 페이지를 하나 할당받습니다. writable은 true로 해서 덮어 쓸 수 있도록 합니다. VM_MARKER_0을 통해서는 스택이 저장 된 메모리 페이지인지 아닌지를 식별할 수 있습니다.
	vm_claim_page(stack_bottom); // 할당 받은 페이지에 바로 물리 프레임을 매핑한다. vm_Claim_page는 bool 값을 반환함.
	if_->rsp = USER_STACK;
	success = true;
